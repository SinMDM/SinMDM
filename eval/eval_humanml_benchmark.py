...
if __name__ == '__main__':
    ...
    eval_files = [os.path.join(args.benchmark_dir, f'{i:04d}', f'eval_{args.num_steps-1:09d}.npy') for i in range(args.benchmark_size)]  #  FIXME -1
    ...
    for sample_i in range(args.benchmark_size):
        for win_size, metrics in eval_dicts[sample_i].items():
    ...
    win_sizes = list(eval_dicts[0].keys())
    metrics_names = list(eval_dicts[0][win_sizes[0]].keys())
    mean_dict = {ws:{m: {
        'mean': np.mean([eval_dicts[sample_i][ws][m]['mean'] for sample_i in range(args.benchmark_size)]),
        'std': np.std([eval_dicts[sample_i][ws][m]['mean'] for sample_i in range(args.benchmark_size)]),
    } for m in metrics_names} for ws in win_sizes}
    print(mean_dict)
    
    np.save(log_file.replace('.log', '.npy'), mean_dict)
    with open(log_file, 'w') as fw:
        fw.write(str(mean_dict))
    for win_size, metrics in mean_dict.items():
        for m, vals in metrics.items():
